...
import { resolveContextosConfigJson } from '../../lib/contextos-home.js';
...
export const localHookSecretCheck: Check = {
  id: 20,
  name: 'LOCAL_HOOK_SECRET present (env or ~/.contextos/config.json)',
  severity: 'yellow',
  async run(ctx) {
    ...
    const configPath = resolveContextosConfigJson(ctx.contextosHome);
    try {
      ...
      return {
        ...
        remediation: 'Run `contextos team login` (when team mode opens) or set LOCAL_HOOK_SECRET in env.',
      };
    } catch {
      ...
      return {
        ...
        remediation:
          'Set LOCAL_HOOK_SECRET in your env (solo mode) or run `contextos team login` (team mode, when GA).',
      };
    }
  },
};
